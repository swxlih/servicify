...
class ShopService {
  ...
  Future<void> registershop(ShopModel user) async {
    ...
    var snap = await _shopCollection.doc(userCredential.user!.uid).get();
  ...
  //update user
  Future<void> updateUser(ShopModel user) async {
    SharedPreferences _pref=await SharedPreferences.getInstance();

    await _shopCollection.doc(user.uid).update({
      'uid': user.uid,
      'phone': user.phone,
      'location': user.location,
      'imgurl': user.imgurl,
  ...
  Future<DocumentSnapshot> loginUser(ShopModel user) async {
    SharedPreferences _pref = await SharedPreferences.getInstance();
  ...
  //   print(city);
  //   QuerySnapshot snap =
  //   await _shopCollection.where('city',isEqualTo: city).where('usertype',isEqualTo:"shop").get();
  //
  //   List<UserModel> data = [];
  //
  ...
  //   await _shopCollection.doc(eventId).get();
  //
  //   if (snapshot.exists) {
  ...
  //
  //   QuerySnapshot snap =
  //   await _shopCollection.where('usertype',isEqualTo:"shop").get();
  //
  //   List<UserModel> data = [];
  //
  //   for (var snapshot in snap.docs) {
